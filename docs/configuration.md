...
| `BB_TRANSCRIPTION`             | `bb-app config`                                    | Optional                | Voice transcription model in `<service>/<model>` format: a plugin-registered AI service (`codex` with the codex plugin; audio up to 5MB) or `openai/<model>` with `OPENAI_API_KEY`. Defaults to `codex/gpt-transcribe`; launches owned by the Arc app default to `arc-voice/default`, which transcribes through Arc's local Voicebox runtime.                                                                                                                            |
