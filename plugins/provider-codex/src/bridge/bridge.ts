...
// Carried the same way as CODEX_POOL_AUTH_TOKEN, via Codex's own
// env_http_headers CLI config — not a URL path segment. A `/pin/<id>` or
// `/auto/<threadId>` segment appended to openai_base_url made the real Codex
// CLI's requests never reach the Account Pooler hub at all (its own client
// silently misrouted them once the base_url had extra path depth); a header
// riding the exact same passthrough as the auth token does not.
const CODEX_POOL_PIN_ENV = "CODEX_ACCOUNT_POOL_PIN";
const CODEX_POOL_THREAD_ID_ENV = "CODEX_ACCOUNT_POOL_THREAD_ID";
  ...
  return {
    ...
    args: [
      ...
      ...(poolPin
        ? [
            "-c",
            `model_providers.bb-account-pool.env_http_headers.x-bb-account-pool-pin=${JSON.stringify(CODEX_POOL_PIN_ENV)}`,
          ]
        : poolThreadId
          ? [
              "-c",
              `model_providers.bb-account-pool.env_http_headers.x-bb-account-pool-thread-id=${JSON.stringify(CODEX_POOL_THREAD_ID_ENV)}`,
            ]
          : []),
    ],
  };
...
function appServerLaunchEnv(
  ...
  const poolPin = envVars?.[CODEX_POOL_PIN_ENV];
  const poolThreadId = envVars?.[CODEX_POOL_THREAD_ID_ENV];
  return {
    ...process.env,
    ...
    ...(poolPin === undefined ? {} : { [CODEX_POOL_PIN_ENV]: poolPin }),
    ...(poolThreadId === undefined
      ? {}
